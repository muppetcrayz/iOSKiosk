...
public class Printer {
    ...
    public var status: PrinterStatus {
        didSet {
            delegate?.printer(printer: self, didChangeStatus: status)
        }
    }
    ...
    public func printData(printData: PrintData, completionHandler completion: @escaping () -> Void) {
        queue.addOperation {
            do {
                let data = try printData.rawData()
                try self.sendData(data: data)
                completion()
            } catch {
                print(error)
            }
